...
public class ProgBat_Positive {
	...
	public void admin_creates_batch_post_request_for_the_lms_with_request_body() throws InvalidFormatException, IOException {
	...
	public void admin_sends_batch_post_https_request_and_request_body_with_endpoint() throws Exception {
	...
	@Then("Admin receives {int} Batch Created Status with response body.")
	public void admin_receives_batch_created_status_with_response_body(Integer int1) {
		Assert.assertEquals(response.statusCode(),201);
		response.prettyPrint();
	}

	@When("Admin sends Batch POST HTTPS Request and request Body with endpoint with missing field")
	public void admin_sends_batch_post_https_request_and_request_body_with_endpoint_with_missing_field() throws Exception {
		response = request.body(batchesmissingpayload.getJsonString()).post(URLs.postcreateendpoint);
	}

	@Then("Admin receives {int} ok for Batch values in req body")
	public void admin_receives_ok_for_batch_values_in_req_body(Integer int1) {
		Assert.assertEquals(response.statusCode(),500);
		response.prettyPrint();
	}

	@Given("Admin creates Batch GET Request for the LMS API")
	public void admin_creates_batch_get_request_for_the_lms_api() throws InvalidFormatException, IOException {
		request.baseUri(URLs.BaseURL);
		request.header("Authorization", "Bearer " + BearerToken.bearertoken());
		request.header("Content-Type","application/json");
	}
	...
	public void admin_sends_batch_get_https_request_with_endpoint_batch_id() {
		response=request.get(URLs.getbatchidendpoint + batchId);
		
	}

	@Then("Admin receives {int} OK Status to reterive Batches with id with response body")
	public void admin_receives_ok_status_to_reterive_batches_with_id_with_response_body(Integer int1) {
		Assert.assertEquals(response.statusCode(),200);
		response.prettyPrint();
	}

	@When("Admin sends Batch GET HTTPS Request with endpoint batchName")
	public void admin_sends_batch_get_https_request_with_endpoint_batch_name() {
		response=request.get(URLs.getbatchnameendpoint + batchName);
	}

	@When("Admin sends Batch GET HTTPS Request with endpoint programId")
	public void admin_sends_batch_get_https_request_with_endpoint_program_id() {
		response=request.get(URLs.getbatchprogramidendpoint);
	}

	@Given("Admin creates Batch PUT Request for the LMS API")
	public void admin_creates_batch_put_request_for_the_lms_api() throws InvalidFormatException, IOException {
		request.baseUri(URLs.BaseURL);
		request.header("Authorization", "Bearer " + BearerToken.bearertoken());
		request.header("Content-Type","application/json");
	}
	...
	public void admin_sends_batch_put_https_request_with_endpoint_batch_id_and_request() throws Exception {
	...
	@Then("Admin receives {int} OK Status for updated Batches with response body.")
	public void admin_receives_ok_status_for_updated_batches_with_response_body(Integer int1) {
		Assert.assertEquals(response.statusCode(),200);
		response.prettyPrint();
	}

	@Given("Admin creates Batch DELETE Request for the LMS API")
	public void admin_creates_batch_delete_request_for_the_lms_api() throws InvalidFormatException, IOException {
		request.baseUri(URLs.BaseURL);
		request.header("Authorization", "Bearer " + BearerToken.bearertoken());
		request.header("Content-Type","application/json");
	}
	...
	public void admin_sends_batch_delete_https_request_with_endpoint_id() {
